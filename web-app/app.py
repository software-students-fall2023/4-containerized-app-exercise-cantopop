...
def upload_image():
    ...
    if data and 'imageData' in data:
        ...
        _, encoded = image_data.split(",", 1)  # Unused variable 'header' removed
...
def edit_note_confirm(title):
    ...
    return render_template('edit_note.html', title=new_title,
                           main_body=new_main_body, message="Note Updated")
...
def search_notes():
    ...
    docs = notes.find({}).sort("title", 1)
    found = [doc for doc in docs if doc["title"].count(keywords)]
    if not found:
